...
for m in np.arange(10,13):
	...
	ds.to_zarr(store='/scratch/cnt0024/hmg2840/albert7a/eNATL60/zarr/zarr_eNATL60-BLBT02-SSU-1h-y'+str(year)+'m'+str(mm), encoding=encoding)
...
for m in np.arange(1,13):
        ...
        encoding = {vname: {'compressor': compressor} for vname in ds.variables}
        ds.to_zarr(store='/scratch/cnt0024/hmg2840/albert7a/eNATL60/zarr/zarr_eNATL60-BLBT02-SSV-1h-y'+str(year)+'m'+str(mm), encoding=encoding)
...
name='/scratch/cnt0024/hmg2840/albert7a/eNATL60/zarr/eNATL60-BLBT02-SSV-1h'
variable='time_counter'
for dname in [f for f in os.listdir(name) if not f.startswith('.')]:
     for fname in [f for f in os.listdir(f'{name}/{variable}') if not f.startswith('.')]:
         os.remove(f'{name}/{variable}/{fname}')
...
ds_z = xr.open_zarr('/scratch/cnt0024/hmg2840/albert7a/eNATL60/zarr/eNATL60-BLBT02-SSU-1h')
ds_z
